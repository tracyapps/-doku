...
## *doku product decisions

- Product name is *doku (wildcard); stardoku.app is a possible domain only.
- Approved brand artwork lives in public/assets/brand: one adaptive full SVG logo that follows the containing light/dark color scheme, plus the simplified square icon for favicons and tiny identity marks. Preserve the dramatic purple grid and high-energy burst rather than rebuilding the mark as text.
- Approved source is public/design/approved-game.png, refined by user feedback: fixed game type/level, quiet in-game navigation, and solid hue cell colors with darker printed motifs; notes remain single colored shapes.
- Themes are expressive skins, not palette swaps: Retro, Comic Book, Doodle, Glass, and Web 2.0 each change the full app surface, typography, board/region geometry, keypad shapes, spacing, depth, and feedback while preserving readable, accessible game states. Comic Book uses wordless spiky impact bubbles behind selected keypad values and matching grid values or notes; Web 2.0 uses rounded 3×3 sectors and beveled value highlights. Jigsaw boards use nine distinct, theme-specific region treatments rather than alternating two fills.
- Keep cell-first selection highlights synchronized with the current square; selecting an empty square clears the previous value highlight. Value-first keeps its armed value for repeated placement. Tapping an already-active keypad value or tapping away from the board/keypad clears its highlight. In value-first, Erase is an armed input tool and clears either the cell value or all cell notes according to the Pen/Notes toggle.
- Pen/Notes toggles and Undo preserve the active keypad value, especially in value-first mode; other controls outside the board/keypad continue to clear it unless explicitly exempted.
- Keep a quiet Back to menu action above the in-game title; returning home preserves the unfinished session and exposes Resume puzzle. Put the compact timer beside puzzle type/level beneath the title, with Settings/More grouped at the other side so long variant names cannot crop actions. Given values are visually quieter; selecting one reveals a lock helper and disables incompatible entry keys so immutability feels intentional.
- Sudoku rows and columns are fixed to nine equal tracks so note content never shifts board height. Keep input-order instructions beside the compact Cell first/Value first control when space allows, with a link to the animated comparison on the shared How to play page.
- Number typography defaults to each theme's own font and weight; Clean, Bookish, Monospace, and explicit weight choices override those defaults. Selected controls must use a readable bold/outline/depth treatment. Selected cells must swap note color for sufficient contrast when the selected surface is dark.
- Reserve red for errors or unmistakably decorative comic ink. Doodle's interactive accent and player-entered values use ink blue. Auto-check entries remains an optional Settings control that marks incorrect pen entries immediately and records assistance.
- Theme fills the complete app surface, including short history pages.
- Preserve the mobile preview runtime. web.html is the separate production website entry and shares the game component, without replacing the preview.
- The public website explains the three puzzle variants, expressive themes, input methods, multidimensional result metrics, deliberate/manual challenge sharing, and least-privilege Discord setup. Keep Privacy and Terms reachable from its footer. Discord application ID is 1548073007950602303.
- Launch Discord sharing is deliberately lightweight: the Discord-managed Activity entry point may announce that someone opened the game, while completed-result sharing is always user-triggered. Use the native Discord share chooser before adding a server/channel picker. Live edited group cards, guild/channel-linked history, and cross-device sync are later authenticated phases, not launch blockers.
- Guest play remains accountless and local. The preferred future sync account is optional Discord sign-in on the web plus automatic Discord identity inside the Activity; do not add passwords, email accounts, or an in-game social graph for launch.
- The public roadmap is GitHub-backed: issues tagged `roadmap` appear by status, while new bug reports and suggestions use structured GitHub Issue Forms and remain off the roadmap until triaged. Keep Roadmap, Suggest an idea, and Report a bug reachable from the public site and the mobile in-game menu. GitHub or Discord accounts must never be required for guest play or browser-local saves.
- Public website pages use canonical trailing-slash paths (`/play/`, `/history/`, `/how/`, `/roadmap/`, `/privacy/`, and `/terms/`) rather than `?page=` routing. Continue accepting the old query URLs as redirects/canonicalization inputs so existing links do not break.
- Never lose existing browser history while testing. Use another origin for isolated QA.
